...
[assembly: Xamarin.Forms.Dependency(typeof(Detector_de_suelos.Droid.TensorflowClassifier))]
namespace Detector_de_suelos.Droid
{
    public class TensorflowClassifier : IClasificador
    {
        ...
        public async Task Classify(byte[] bytes)
        {
            ...
            var result = new List<Prediccion>();

            for (var i = 0; i < labels.Count; i++)
            {
                var label = labels[i];
                result.Add(new Prediccion(label, classificationResult[0][i]));
            }
